...
class ResidualUnetAE(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.n_blocks):
            ...
            output.append(x_out.clone())
        # return x_out, output
        return x_out, torch.cat(output, dim=-1)
...
class ResidualVAE(nn.Module):
    ''' Residual variational autoencoder using fc layers
        layers should be something like [128, 64, 32]
        eg:[128,64,32]-> add: [(input_dim, 128), (128, 64), (64, 32), (32, 64), (64, 128), (128, input_dim)]
                          concat: [(input_dim, 128), (128, 64), (64, 32), (32, 64), (128, 128), (256, input_dim)]
    '''
    ...
    def forward(self, x):
        ...
        return self.transition(x_in + x_out), latents, mus, logvars

# 重构损失和KL散度
def vae_loss(reconstructed, original, mus, logvars):
